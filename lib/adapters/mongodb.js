    ...
    adapter._init = function (options) {
        ...
        db.on('error', function (err) {
            if (err.message != lastErrorMessage) {
                ...
                /**
                 * If the error happens on startup then reconnection is attempted very fast and logs would get polluted
                 */
            }
            if (err && db.db) { // couldn't connect
                // hack the driver to allow re-opening after initial network error
                db.db.close();
            }

            lastErrorMessage = err && err.message || err;
            // retry if desired
            connect();
        });
