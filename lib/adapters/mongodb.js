...
function Adapter() {
    ...
    adapter._init = function (options) {
      ...
      var oplogReadsFromSecondary = options.oplogReadsFromSecondary || false;
      ...
      if(oplogReadsFromSecondary) {
        console.log('oplog from secondary true');
        gooseOpt.replset = gooseOpt.replset || {};
        var readsFromSecondary = { readPreference: 'ReadPreference.SECONDARY_PREFERRED' };
        _.assign(gooseOpt.rplset, readsFromSecondary);
      }

      console.log('>>>> gooseOpt', JSON.stringify(gooseOpt, null, 2));
